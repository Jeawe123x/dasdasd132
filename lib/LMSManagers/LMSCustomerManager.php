...
class LMSCustomerManager extends LMSManager implements LMSCustomerManagerInterface
{
    ...
    public function customerStats()
    {
        ...
        $tmp = $this->db->GetRow(
            'SELECT SUM(a.value)*-1 AS debtvaluewithoutvindication, COUNT(*) AS debtwithoutvindication
            FROM (
                SELECT SUM(value) AS value
                FROM cash
                LEFT JOIN customerview ON (customerid = customerview.id)
                WHERE deleted = 0 and status <> 5
                GROUP BY customerid
                HAVING SUM(value) < 0
            ) a'
        );
        ...
        if ($result = $this->db->GetRow('SELECT c.*, '
           ...
           if (isset($result['accounts']) ) {
               foreach ($result['accounts'] as $account) {
                   if ($account["type"] == 80 or $account["type"] == 64) $result['bankaccount'] = $account['account'];
               }
           }
