...
//			$h_cell = $this->backend->getStringHeight($h_width[$item], $heads[$item], true, false, 0, 1);
		...
		/* invoice data */
		$i = 1;
//		file_put_contents("/tmp/invoice.log",var_export($this->data["content"],true),FILE_APPEND);
		...
		foreach ($this->data['content'] as $item) {
			...
			$this->backend->Cell($h_width['total'], $h, moneyf($item['total']), 1, 0, 'R', 0, '', 1);
			$this->backend->Ln();
			if ($item['taxlabel'] == "OO") $this->odwrocony = true;
			$i++;
		}
...
#		$tmp = str_replace('%bankaccount', implode("\n", $accounts), $tmp);
		$tmp = str_replace('%bankaccount', end($accounts), $tmp);
...
//			$bankaccount = trans('Bank account:') .'<br>' . implode('<br>', $accounts);
			$bankaccount = trans('Bank account:') .'<br>' . end($accounts).'<br>';
			$this->backend->SetFont('arial', 'B', 8);
			$this->backend->writeHTMLCell('', '', 120,  $oldy + round(($y - $oldy) / 2), $bankaccount, 0, 1, 0, true, 'L');
		}

		if (ConfigHelper::checkValue(ConfigHelper::getConfig('invoices.customer_credentials', true))) {
			$pin = '<b>' . trans('Customer ID: $a', sprintf('%04d', $this->data['customerid'])) . '</b> / ';
			$pin .= '<b>PIN: ' . (strlen($this->data['customerpin']) < 4 ? sprintf('%04d', $this->data['customerpin']) : $this->data['customerpin']) . '</b><br>';

			$this->backend->SetFont('arial', 'B', 8);
			$this->backend->writeHTMLCell('', '', '', $oldy + round(($y - $oldy + 20) / 2), $pin, 0, 1, 0, true, 'L');
		}
	...
	protected function invoice_balance() {
		...
		if ($this->odwrocony) $this->backend->writeHTMLCell(0, 0, '', '', "<b>Do rozliczenia podatku VAT zobowiązany jest nabywca (odwrotne obciążenie)</b>", 0, 1, 0, true, 'C');
		$this->backend->SetFont('arial', '', 7);
